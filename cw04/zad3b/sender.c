...
int catcher_id;
int signals;
void send_signal(int end)
{
    union sigval sigv;
    if(end == 0)
    {
        if (type == SIGQUEUE)
            sigqueue(catcher_id, SIGNAL_SEND, sigv);
        else
    ...
    {
        if (type == SIGQUEUE)
            sigqueue(catcher_id, SIGNAL_SEND_END, sigv);
        else
...
{
    printf("Sender got %d signals and should got %d\n", SIGNAL_REACHED, signals);
    end = 1;
}
